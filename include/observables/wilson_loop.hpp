...
template <size_t rank, size_t Nc, class RNG> struct WLoop_munu {
  ...
  WLoop_munu(const GaugeFieldType &g_in, FieldType &Wmunu_per_site,
             ...
             const real_t delta, const real_t epsilon1, const real_t epsilon2,
             const RNG &rng,
             const IndexArray<rank> &dimensions)
      : g_in(g_in), Wmunu_per_site(Wmunu_per_site), mu(mu), nu(nu), Lmu(Lmu),
        Lnu(Lnu), multihit(multihit), beta(beta), delta(delta),
        epsilon1(epsilon1), epsilon2(epsilon2), rng(rng), dimensions(dimensions) {}
    ...
    if (multihit > 1) {
      Kokkos::parallel_for(
          ...
          WLoop_munu<rank, Nc, RNG>(g_in, Wmunu_per_site, mu, nu, Lmu, Lnu,
                                    multihit, beta, delta, epsilon1, epsilon2,
                                    rng, end));
    } else {
      Kokkos::parallel_for(
          Policy<rank>(start, end),
...
void WilsonLoop_temporal(
  ...
  if constexpr (Nd > 2) {
    ...
    WilsonLoop_mu_nu<rank, Nc>(g_in, 1, Nd - 1, L_T_pairs, Wmunu_vals,
                               multihit, beta, delta, epsilon1, epsilon2, rng,
                               normalize);
