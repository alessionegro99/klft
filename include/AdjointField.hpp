...
public:
  ...
  AdjointField(const int &_LX, const int &_LY, const int &_LZ, const int &_LT) {
    ...
    for (int i = 0; i < 2 * Nc - 1; ++i) {
      for (int mu = 0; mu < Ndim; ++mu) {
        this->adjoint[mu][i] = DeviceView(
            Kokkos::view_alloc(Kokkos::WithoutInitializing, "adjoint"), LX, LY,
            LZ, LT);
      }
    }
  ...
  KOKKOS_INLINE_FUNCTION int get_volume() const {
    return this->LX * this->LY * this->LZ * this->LT;
  }

  KOKKOS_INLINE_FUNCTION int get_size() const {
    return this->LX * this->LY * this->LZ * this->LT * Ndim * (2 * Nc - 1);
  }
  ...
  KOKKOS_INLINE_FUNCTION Adjoint get_adjoint(const int &x, const int &y,
...
#pragma unroll
  ...
  KOKKOS_INLINE_FUNCTION Adjoint get_adjoint(const Kokkos::Array<int, 4> &site,
                                             const int &mu) const {
    Kokkos::Array<T, 2 * Nc - 1> adj;
#pragma unroll
    for (int i = 0; i < 2 * Nc - 1; i++) {
  ...
  KOKKOS_INLINE_FUNCTION void set_adjoint(const int &x, const int &y,
                                          const int &z, const int &t,
                                          const int &mu,
                                          const Adjoint &U) const {
#pragma unroll
    for (int i = 0; i < 2 * Nc - 1; i++) {
  ...
  KOKKOS_INLINE_FUNCTION void set_adjoint(const Kokkos::Array<int, 4> &site,
                                          const int &mu, const Adjoint &U) {
#pragma unroll
    for (int i = 0; i < 2 * Nc - 1; i++) {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(set_zero_s, const int &x, const int &y,
                                         const int &z, const int &t,
                                         const int &mu) const {
#pragma unroll
    for (int i = 0; i < 2 * Nc - 1; i++) {
  ...
  void set_zero() {
    auto BulkPolicy = Kokkos::MDRangePolicy<set_zero_s, Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0}, {this->LX, this->LY, this->LZ, this->LT, Ndim});
