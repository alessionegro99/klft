...
public:
  ...
  HamiltonianField(const GaugeField<T, Group, Ndim, Nc> _gauge_field,
                   const AdjointField<T, Adjoint, Ndim, Nc> _adjoint_field)
  ...
  void update_momentum(AdjointField<T, Adjoint, Ndim, Nc> deriv, const T dtau) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0},
        {adjoint_field.get_max_dim(0), adjoint_field.get_max_dim(1),
         adjoint_field.get_max_dim(2), adjoint_field.get_max_dim(3),
         adjoint_field.get_Ndim()});
    Kokkos::parallel_for(
        "update_momentum", BulkPolicy,
  ...
  void update_gauge(const T dtau) {
    ...
    Kokkos::parallel_for(
        "update_gauge", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int x, const int y, const int z, const int t,
                            const int mu) {
          Group U = exp(dtau * adjoint_field.get_adjoint(x, y, z, t, mu)) *
                    gauge_field.get_link(x, y, z, t, mu);
  ...
  T kinetic_energy() {
    ...
    Kokkos::parallel_reduce(
        "kinetic_energy", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int x, const int y, const int z, const int t,
                            const int mu, T &local_sum) {
          Adjoint tmp = adjoint_field.get_adjoint(x, y, z, t, mu);
