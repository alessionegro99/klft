...
template <typename T, class Group, int Ndim = 4, int Nc = 3> class GaugeVector {
public:
  ...
  GaugeVector(const Kokkos::Array<int, 4> &_dims) {
    ...
    for (int i = 0; i < Nc * Nc; ++i) {
      this->vector[i] =
          DeviceView(Kokkos::view_alloc(Kokkos::WithoutInitializing, "vector"),
                     LX, LY, LZ, LT);
    }
  ...
  void absorb_direction(const GaugeField<T, Group, Ndim, Nc> &gauge,
                        const int &mu) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                          this->get_max_dim(2), this->get_max_dim(3), Nc * Nc});
    Kokkos::parallel_for(
        "absorb_direction", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int &x, const int &y, const int &z,
                            const int &t, const int &i) {
...
#pragma unroll
  ...
  KOKKOS_INLINE_FUNCTION Group
  get_link(const Kokkos::Array<int, 4> &site) const {
    Kokkos::Array<Kokkos::complex<T>, Nc * Nc> link;
#pragma unroll
    for (int i = 0; i < Nc * Nc; i++) {
  ...
  KOKKOS_INLINE_FUNCTION void set_link(const int &x, const int &y, const int &z,
                                       const int &t, const Group &U) const {
#pragma unroll
    for (int i = 0; i < Nc * Nc; i++) {
...
#pragma unroll
  ...
  void UxU(const GaugeVector<T, Group, Ndim, Nc> &in1,
           const GaugeVector<T, Group, Ndim, Nc> &in2) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<4>>(
        {0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                       this->get_max_dim(2), this->get_max_dim(3)});
    Kokkos::parallel_for(
  ...
  void UxUdag(const GaugeVector<T, Group, Ndim, Nc> &in1,
              const GaugeVector<T, Group, Ndim, Nc> &in2) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<4>>(
        {0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                       this->get_max_dim(2), this->get_max_dim(3)});
  ...
  void shift_plus(const GaugeVector<T, Group, Ndim, Nc> &tmp, const int &mu,
                  const int &dist) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                          this->get_max_dim(2), this->get_max_dim(3), Nc * Nc});
    Kokkos::parallel_for(
        "shift_plus", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int &x, const int &y, const int &z,
                            const int &t, const int &i) {
          Kokkos::Array<int, 4> site = {x, y, z, t};
          site[this->array_dims[mu]] =
              (site[this->array_dims[mu]] + dist) % this->dims[mu];
          tmp.vector[i](x, y, z, t) =
              this->vector[i](site[0], site[1], site[2], site[3]);
        });
    this->copy(tmp);
  }

  void shift_minus(const GaugeVector<T, Group, Ndim, Nc> &tmp, const int &mu,
                   const int &dist) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                          this->get_max_dim(2), this->get_max_dim(3), Nc * Nc});
    Kokkos::parallel_for(
        "shift_minus", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int &x, const int &y, const int &z,
                            const int &t, const int &i) {
          Kokkos::Array<int, 4> site = {x, y, z, t};
          site[this->array_dims[mu]] =
  ...
  T sum_retrace() {
    ...
    Kokkos::parallel_reduce(
        "sum_retrace", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int &x, const int &y, const int &z,
                            const int &t, T &local_sum) {
          Group tmp = this->get_link(x, y, z, t);
  ...
  void wilson_line(GaugeVector<T, Group, Ndim, Nc> &Umu, const int &mu,
                   const int &Lmu, const bool &reverse = false) {
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<4>>(
        {0, 0, 0, 0}, {this->get_max_dim(0), this->get_max_dim(1),
                       this->get_max_dim(2), this->get_max_dim(3)});
    Kokkos::parallel_for(
        "wilson_line", BulkPolicy,
        KOKKOS_CLASS_LAMBDA(const int &x, const int &y, const int &z,
                            const int &t) {
          Kokkos::Array<int, 4> site = {x, y, z, t};
...
T get_W_loop_mu_nu(GaugeField<T, Group, Ndim, Nc> &gauge, const int &mu,
  ...
  T W = U1.sum_retrace();
  if (Normalize)
    W /= (gauge.get_volume() * gauge.get_Nc());
  return W;
}
