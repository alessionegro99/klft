...
public:
  ...
  GaugeMonomial(T _beta, unsigned int _time_scale)
      : Monomial<T, Group, Adjoint, Ndim, Nc>::Monomial(_time_scale) {
    beta = _beta;
  ...
  void heatbath(HamiltonianField<T, Group, Adjoint, Ndim, Nc> h) override {
    Monomial<T, Group, Adjoint, Ndim, Nc>::H_old =
        -(beta / T(h.gauge_field.get_Nc())) *
        h.gauge_field.get_plaquette(false);
  }

  void accept(HamiltonianField<T, Group, Adjoint, Ndim, Nc> h) override {
    Monomial<T, Group, Adjoint, Ndim, Nc>::H_new =
        -(beta / T(h.gauge_field.get_Nc())) *
        h.gauge_field.get_plaquette(false);
  }
  ...
  void derivative(AdjointField<T, Adjoint, Ndim, Nc> deriv,
    ...
    auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
        {0, 0, 0, 0, 0},
        {h.gauge_field.get_max_dim(0), h.gauge_field.get_max_dim(1),
         h.gauge_field.get_max_dim(2), h.gauge_field.get_max_dim(3),
         h.gauge_field.get_Ndim()});
    Kokkos::parallel_for(
        "derivative", BulkPolicy,
