...
KOKKOS_FORCEINLINE_FUNCTION void heatbath_link(SUN<2> &link,
  ...
  if (p > 1.0e-13) {
    ...
    link = rand_matrix_p0_SU2(p0, generator);
    link *= matrix2;
  } else {
    link = rand_matrix_SU2(generator);
  }
}
...
#pragma unroll
    for (index_t j = i + 1; j < Nc; ++j) {
      ...
      if (xi > 1.0e-13) {
        ...
        w = conj(u);
        v = rand_matrix_p0_SU2(p0, generator);
        w *= v;
      } else {
        w = rand_matrix_SU2(generator);
      }
